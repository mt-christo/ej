...
os.chdir("/root/bot/sample-market-maker/")

from market_maker import bitmex
from market_maker import settings_sp as settings
...
class ExchangeInterface:
    def __init__(self, symbol_in, dry_run=False):
    ...
    def get_higher_price(self, price):
        return self.tickSize * int(price/self.tickSize) + self.tickSize
    
    def get_lower_price(self, price):
        return self.tickSize * int(price/self.tickSize)
...
class OrderManager:
    def __init__(self):
        self.exch1 = ExchangeInterface(settings.SYMBOL1, settings.DRY_RUN)
        self.exch2 = ExchangeInterface(settings.SYMBOL2, settings.DRY_RUN)
        # Once exchange is created, register exit handler that will always cancel orders
        # on any error.
        atexit.register(self.exit)
        signal.signal(signal.SIGTERM, self.exit)

        logger.info("Using symbol 1: %s." % self.exch1.symbol)
        logger.info("Using symbol 2: %s." % self.exch2.symbol)
    ...
    def init(self):
        ...
        self.instr1 = self.exch1.get_instrument()
        self.starting_qty1 = self.exch1.get_delta()
        self.running_qty1 = self.starting_qty1

        self.instr2 = self.exch2.get_instrument()
        self.starting_qty2 = self.exch2.get_delta()
        self.running_qty2 = self.starting_qty2

        self.reset()

    def reset(self):
        self.exch1.cancel_all_orders()
        self.exch2.cancel_all_orders()
...
#        self.place_orders()
    ...
    def print_status(self):
        ...
        margin1 = self.exch1.get_margin()
        position1 = self.exch1.get_position()
        self.running_qty1 = self.exch1.get_delta()
        tickLog1 = self.exch1.get_instrument()['tickLog']
        self.start_XBt1 = margin1["marginBalance"]

        margin2 = self.exch2.get_margin()
        position2 = self.exch2.get_position()
        self.running_qty2 = self.exch2.get_delta()
        tickLog2 = self.exch2.get_instrument()['tickLog']
        self.start_XBt2 = margin2["marginBalance"]

        logger.info("Current XBT Balance (1): %.6f" % XBt_to_XBT(self.start_XBt1))
        logger.info("Current Contract Position (1): %d" % self.running_qty1)

        logger.info("Current XBT Balance (2): %.6f" % XBt_to_XBT(self.start_XBt2))
        logger.info("Current Contract Position (2): %d" % self.running_qty2)

        if settings.CHECK_POSITION_LIMITS:
            logger.info("Position limits: %d/%d" % (settings.MIN_POSITION, settings.MAX_POSITION))
            
        if position1['currentQty'] != 0:
            logger.info("Avg Cost Price (1): %.*f" % (tickLog1, float(position1['avgCostPrice'])))
            logger.info("Avg Entry Price (1): %.*f" % (tickLog1, float(position1['avgEntryPrice'])))
        logger.info("Contracts Traded This Run (1): %d" % (self.running_qty1 - self.starting_qty1))
        logger.info("Total Contract Delta (1): %.4f XBT" % self.exch1.calc_delta()['spot'])

        if position2['currentQty'] != 0:
            logger.info("Avg Cost Price (2): %.*f" % (tickLog2, float(position2['avgCostPrice'])))
            logger.info("Avg Entry Price (2): %.*f" % (tickLog2, float(position2['avgEntryPrice'])))
        logger.info("Contracts Traded This Run (2): %d" % (self.running_qty2 - self.starting_qty2))
        logger.info("Total Contract Delta (2): %.4f XBT" % self.exch2.calc_delta()['spot'])
        
    def get_ticker(self):
        ticker1 = self.exch1.get_ticker()
        tickLog1 = self.exch1.get_instrument()['tickLog']

        ticker2 = self.exch2.get_ticker()
        tickLog2 = self.exch2.get_instrument()['tickLog']

        # Set up our buy & sell positions as the smallest possible unit above and below the current spread
        # and we'll work out from there. That way we always have the best price but we don't kill wide
        # and potentially profitable spreads.
        self.start_position_buy1 = ticker1["buy"] + self.instr1['tickSize']
        self.start_position_sell1 = ticker1["sell"] - self.instr1['tickSize']
        self.start_position_buy2 = ticker2["buy"] + self.instr2['tickSize']
        self.start_position_sell2 = ticker2["sell"] - self.instr2['tickSize']
    ...
    def sanity_check(self):
        ...
        self.exch1.check_if_orderbook_empty()
        self.exch2.check_if_orderbook_empty()

        # Ensure market is still open.
        self.exch1.check_market_open()
        self.exch2.check_market_open()
    ...
    def exit(self):
        ...
        logger.info("no - orders will not be affected")
        try:
...
def run():
    ...
    try:
        ...
        prc1 = om.exch1.get_ticker()["buy"]
        prc2 = om.exch2.get_ticker()["sell"]


        
        om.exch1.create_bulk_orders([{'price': om.exch1.get_higher_price(prc1*1.01), 'orderQty': 5, 'side': "Buy"}])
        om.exch2.create_bulk_orders([{'price': om.exch2.get_lower_price(prc2*0.99), 'orderQty': 5, 'side': "Sell"}])
